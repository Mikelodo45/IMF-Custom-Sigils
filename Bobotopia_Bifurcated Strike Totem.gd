...
func handle_event(event: String, params: Array):
	...
	if event == "card_summoned" and params[0] == card:
		...
		if "sigils" in targetCard and len(targetCard.card_data.sigils) < 3 and not "Bifurcated Strike" in targetCard.card_data.sigils:
			var new_data = targetCard.card_data.duplicate(true)
			new_data.sigils.append("Bifurcated Strike")
